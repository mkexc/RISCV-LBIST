...
set riscvNetlist "../standalone/riscv_core.ddc"
...
# output netlist name
set coreNetOutNoHier "riscv_core_scan_nohier$chains.v"
set coreNetOut "riscv_core_scan$chains.v"
# output ddc name
set coreDDCOutNoHier "riscv_core_scan_nohier$chains.ddc"
set coreDDCOut "riscv_core_scan$chains.ddc"

# output stil file name
set coreStilOutNoHier "riscv_core_scan_nohier$chains.spf"
...
set_testability_configuration \
	...
	-clock_signal clk_i \
	-test_points_per_scan_cell 32

set_testability_configuration -target random_resistant -effort high -random_pattern_count 5000 -max_test_points 4000
...
#report_area
write -format verilog -output $outDir$coreNetOutNoHier
write -format ddc -output $outDir$coreDDCOutNoHier
write -hierarchy -format verilog -output $outDir$coreNetOut
write -hierarchy -format ddc -output $outDir$coreDDCOut
